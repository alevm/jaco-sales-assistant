...
function calcMargin(
  ...
  const feePercent = marketplace ? MARKETPLACE_FEES[marketplace] : 0;
...
export default function NewItemPage() {
  ...
  const handleFiles = useCallback(async (files: FileList | File[]) => {
    ...
    if (arr.length === 0) {
      setUploadError("Seleziona file immagine validi.");
      return;
    }
      ...
      setImages((prev) => {
        ...
        for (let i = updated.length - arr.length; i < updated.length; i++) {
          if (pathIdx < paths.length) {
            updated[i] = { ...updated[i], serverPath: paths[pathIdx] };
            pathIdx++;
          }
        }
        return updated;
  ...
  const handleRecognize = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || "Riconoscimento fallito");
      }
      ...
      const msg = e instanceof Error ? e.message : "Riconoscimento fallito";
  ...
  /* ---------- Step 4: Save & Generate Descriptions ---------- */
  ...
  const handleGenerateDescriptions = async () => {
    ...
    try {
      ...
      if (!resIt.ok) throw new Error("Generazione descrizione IT fallita");
      const dataIt = await resIt.json();
      setDescriptionIt(dataIt.description_it);
      ...
      if (!resEn.ok) throw new Error("Generazione descrizione EN fallita");
      const dataEn = await resEn.json();
      setDescriptionEn(dataEn.description_en);
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : "Errore generazione descrizioni";
      setDescError(msg);
    } finally {
  ...
  const handleFinalSave = async () => {
    if (savedItemId) {
      // Item already saved during description generation — update it
      setSaving(true);
      try {
        ...
        // Redirect anyway since core data was saved
  ...
  const canProceedStep2 = recognition !== null;
            ...
            {i < STEPS.length - 1 && (
              <div
                className={`w-6 h-0.5 mx-1 ${
                  s.num < step ? "bg-amber-400" : "bg-stone-200"
                }`}
              />
            )}
          </div>
      ...
      {step === 1 && (
        <div className="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm">
          ...
          >
            ...
            />
            <div className="text-4xl mb-3 text-stone-400">
              {uploading ? (
            ...
            <p className="text-stone-600 font-medium">
              {uploading ? "Caricamento..." : "Trascina le foto qui o clicca per selezionare"}
            </p>
            <p className="text-stone-400 text-sm mt-1">JPG, PNG, WebP &mdash; max 5MB per file</p>
          </div>
          ...
          {images.length > 0 && (
            <div className="mt-5 grid grid-cols-4 gap-3">
              {images.map((img, i) => (
                <div
                  key={i}
                  className="relative group rounded-xl overflow-hidden border border-stone-200 aspect-square bg-stone-100"
                >
                  ...
                  {img.serverPath && (
                    <div className="absolute bottom-1.5 left-1.5 w-4 h-4 rounded-full bg-green-500 border-2 border-white" />
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm">
          ...
          <div className="flex gap-2 mb-5 overflow-x-auto pb-2">
            {images.map((img, i) => (
              <div
                key={i}
                className="w-20 h-20 rounded-lg overflow-hidden border border-stone-200 flex-shrink-0"
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={img.preview}
          ...
          {!recognition && (
            <button
              ...
              disabled={recognizing || uploadedPaths.length === 0}
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { label: "Condizione", value: condition, set: setCondition },
                ].map((field) => (
                  <label key={field.label} className="block">
                    ...
                    <input
                      ...
                      className="mt-1 block w-full rounded-lg border border-stone-300 bg-stone-50 px-3 py-2 text-sm text-stone-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                    />
                  </label>
                ))}
              </div>
                ...
                <div>
                  ...
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {tags.map((tag, i) => (
                      <span
                        key={i}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs bg-stone-100 text-stone-700 border border-stone-200"
                      >
              ...
              <div className="flex justify-between mt-4">
                <button
                  onClick={() => setStep(1)}
                  className="px-4 py-2 rounded-xl text-sm font-medium text-stone-600 border border-stone-300 hover:bg-stone-50 transition-colors"
                >
                  &larr; Indietro
                </button>
                <button
                  onClick={() => setStep(3)}
                  className="px-5 py-2 rounded-xl bg-amber-600 text-white font-medium text-sm hover:bg-amber-700 transition-colors"
                >
                  Avanti &rarr;
                </button>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <label className="block">
                ...
                <select
                  ...
                  className="mt-1 block w-full rounded-lg border border-stone-300 bg-stone-50 px-3 py-2 text-sm text-stone-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                >
                  <option value="vinted">Vinted (0% fee)</option>
                  <option value="ebay">eBay (13% fee)</option>
                </select>
              </label>
                ...
                <div className="grid grid-cols-4 gap-4 text-center text-sm">
                  <div>
                    ...
                    <p className="text-stone-700 font-semibold mt-1">
                      &euro;{margin.platformFee.toFixed(2)}
                    </p>
                  </div>
                  <div>
                    <p className="text-stone-400 text-xs uppercase">Ricavo netto</p>
                    <p className="text-stone-700 font-semibold mt-1">
                      &euro;{margin.netRevenue.toFixed(2)}
                    </p>
                  </div>
                  <div>
                    ...
                    <p
                      className={`font-bold mt-1 ${
                        margin.netMargin >= 0 ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      &euro;{margin.netMargin.toFixed(2)}
                    </p>
                  </div>
                  <div>
                    ...
                    <p
                      className={`font-bold mt-1 ${
                        margin.marginPercent >= 0 ? "text-green-600" : "text-red-600"
                      }`}
                    >
            ...
            <div>
              ...
              <div className="relative">
                <div className="bg-stone-50 rounded-xl border border-stone-200 p-4 min-h-[120px] text-sm text-stone-700 whitespace-pre-wrap">
                  {descTab === "it"
                    ? descriptionIt || "Nessuna descrizione generata"
                    : descriptionEn || "No description generated"}
                </div>
                <button
                  onClick={() => {
                    const text =
                      descTab === "it" ? descriptionIt : descriptionEn;
                    if (text) copyToClipboard(text, descTab);
                  }}
